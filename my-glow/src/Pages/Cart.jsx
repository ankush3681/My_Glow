...
import {
  Box,
  Center, Heading, SimpleGrid,Image,Button , HStack, Text, Icon, useToast
    ...
    <Box style={{marginTop:'8%'}}>
      <Box display={'grid'} gridTemplateColumns= {{base:'repeat(1,1fr)',sm:"73% 25%"}} gap={'1%'} m={1} ml={3} mb={'70px'}>
        <Box>
          {
            ...
            {cartItems.map((curItem)=>(
              <HStack key={curItem._id} border='1px solid #BDBDBD' height={{base:'180px',sm:'200px'}} mx={4} my={4}>
                  <Box width={{base:'30%',sm:'20%'}} height={{base:'180px',sm:'200px'}} border='1px solid #BDBDBD'  >
                    <Image src={curItem.image1} width={'100%'} height={'100%'} alt='image' />
                  </Box>
                  <Box width={{base:'68%',sm:'78%'}}  px={15} >
                    <Heading size='sm'>{curItem.product_name}</Heading>
                    <Text py='2'>
                      {curItem.brand}
                    </Text>
                    <Box ml={{base:"0px",sm:"0px"}} display={{base:'flex',sm:'flex'}} justifyContent={"left"} >
                      <Text fontWeight={'semibold'} >Rs. {curItem.price}</Text>
                    </Box>
                    ...
                    <input type="text" style={{padding:'.5px'}} placeholder={curItem.quantity} disabled />
                    <i className="fas fa-plus add" onClick={() => dispatch(incrementQuantity(curItem.id))}>+</i>
                   </div>
                      <Icon _hover={{cursor:'pointer'}} onClick={()=>dispatch(removeFromCart(curItem.id))} mr={{base:"20px" ,sm:"600px"}} fontSize={'35px'} mt={4} as={RiDeleteBin6Line} />
                    </HStack>
                  </Box>
              </HStack>
            ))}
        ...
        <Box border='1px solid #EEEEEE' bg='#FAFAFA' px={4} pt={4} mt={5}>
                ...
                <Link to='/address' >
                  <Button mt={2} rounded={'none'} 
                  // bgGradient="linear(to-r, #F8BBD0, #b3d4fc)"
                  bg="#e11299"
                   color='white' width={'100%'} isDisabled={grandTotal==0} >
                    PROCEED TO PAY
                  </Button>
                </Link>
        </Box>
